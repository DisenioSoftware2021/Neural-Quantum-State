...
def test_gaussian_nqs_exponential_argument(hidden, dim, partic, visible, expect_exp):
...
input_values_psi = [
    (2, 2, 1, vectorr_list[0], expected_list_exponential[0], expected_list_psi[0]),
    (3, 3, 1, vectorr_list[1], expected_list_exponential[1], expected_list_psi[1]),
    (4, 2, 2, vectorr_list[2], expected_list_exponential[2], expected_list_psi[2]),
]


@pytest.mark.parametrize("hidden,dim,partic,visible,q,expect_psi", input_values_psi)
...
input_values_sigmoid = [
    (2, 2, 1, vectorr_list[0], expected_list_exponential[0], expected_list_sigmoid[0]),
    (3, 3, 1, vectorr_list[1], expected_list_exponential[1], expected_list_sigmoid[1]),
    (4, 2, 2, vectorr_list[2], expected_list_exponential[2], expected_list_sigmoid[2]),
]


@pytest.mark.parametrize("hidden,dim,partic,visible,q,expect_sig", input_values_sigmoid)
...
@pytest.mark.parametrize("hidden,dim,partic,q,expect_der_sig", input_values_der_sigmoid)
...
expected_list_laplacian = [-15582.145635791683, -24819.88750141783, -38045.44472907044]
input_values_laplacian = [
    (2, 2, 1, vectorr_list[0], expected_list_exponential[0], expected_list_laplacian[0]),
    (3, 3, 1, vectorr_list[1], expected_list_exponential[1], expected_list_laplacian[1]),
    (4, 2, 2, vectorr_list[2], expected_list_exponential[2], expected_list_laplacian[2])
]
...
expected_list_laplacian_alfa = [
    [
        96.09572965, 97.28013178, 0.5, 0.5, 41.74908153,
        41.74908153, 29.82770135, 29.82770135,
    ],
    [
        9.72801318e01, 1.04806913e02, 9.46449567e01,
        5.00000000e-01, 5.00000000e-01, 4.22912355e-18,
        4.17490815e01, 4.17490815e01, 3.53124048e-16,
        2.98277013e01, 2.98277013e01, 2.52290069e-16,
        1.44431621e01, 1.44431621e01, 1.22163834e-16,
    ],
    [
        1.04806913e02, 9.46449567e01, 9.64927217e01,
        9.64751327e01, 4.51049654e-02, 2.30671305e-03,
        4.99998333e-01, 1.64095975e-24, 3.76618176e00,
        1.92606302e-01, 4.17489424e01, 1.37017124e-22,
        2.69075488e00, 1.37607896e-01, 2.98276019e01,
        9.78921144e-23, 1.30291665e00, 6.66324608e-02,
        1.44431139e01, 4.74012952e-23, 1.93732911e-01,
        9.90769485e-03, 2.14757138e00, 7.04817985e-24,
    ],
]
input_values_laplacian_alfa = [
    (2, 2, 1, vectorr_list[0], expected_list_exponential[0], expected_list_laplacian_alfa[0]),
    (3, 3, 1, vectorr_list[1], expected_list_exponential[1], expected_list_laplacian_alfa[1]),
    (4, 2, 2, vectorr_list[2], expected_list_exponential[2], expected_list_laplacian_alfa[2]),
]

@pytest.mark.parametrize(
    "hidden,dim,partic,visible,q,expect_lap_alfa", input_values_laplacian_alfa
)
def test_gaussian_nqs_laplacian_alfa(hidden, dim, partic, visible, q, expect_lap_alfa):
    qs = NQS.GaussianNQS(hidden, dim, partic, 0.1, 1.0, seed=100)
    qs.sigmoid(q)
...
def test_gaussian_nqs_inverse_distance(hidden, dim, partic, visible, expect_inverse):
    qs = NQS.GaussianNQS(hidden, dim, partic, 0.1, 1.0, seed=100)
